...
export default function EmailDetails({ email, onReply, onForward }) {
  ...
  useEffect(() => {
    ...
    emailService
      .getSummary(email.gmail_id)
      .then((data) => {
        if (!cancelled) setSummary(data.summary);
      })
      .catch(() => {
        if (!cancelled) setSummaryError("Couldn't generate summary.");
      })
      .finally(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="sticky top-0 bg-white border-b px-8 py-6 z-10">
        <div className="flex justify-between items-start">
          <div className="flex gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-900">{email.subject}</h1>
              <p className="text-gray-600 mt-2">{email.sender}</p>
            </div>
          </div>
      ...
      <div className="mx-8 mt-6 rounded-2xl border border-purple-200 bg-gradient-to-r from-purple-50 to-blue-50 p-6">
        <div className="flex items-center gap-3 mb-3">
          <Sparkles className="text-purple-600" />
          <h2 className="font-semibold text-purple-700">AI Summary</h2>
        </div>
      ...
      <div className="flex gap-2 flex-wrap px-8 mt-6">
        {email.labels?.map((label) => (
          <span key={label} className="px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-sm">
            {label}
          </span>
        ))}
      </div>
      ...
      <div className="border-t px-8 py-5 flex justify-between items-center bg-white">
        <p className="text-sm text-gray-500">Received on {email.date}</p>

        <div className="flex gap-3">
          <button
            onClick={onReply}
            className="rounded-xl bg-blue-600 px-6 py-2 text-white hover:bg-blue-700 transition"
          >
            Reply
          </button>
          ...
          <button
            onClick={onForward}
            className="rounded-xl border px-6 py-2 hover:bg-gray-100 transition"
          >
            Forward
          </button>
        </div>
